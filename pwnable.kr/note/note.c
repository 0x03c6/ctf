...
void create_note(){
	...
	for(i=0; i<256; i++){
		if(mem_arr[i] == NULL){
      // memory is executable, we can write shellcode to these pages...
...
void write_note(){
  // unsigned, so no integer underflow...
	unsigned int no;	
	printf("note no?\n");
	scanf("%d", &no);	
	clear_newlines();	
  ...
  // gets(), we get an infinite read but the mapped page is only 4096 bytes?
  // can we write past the bounds of our page??
	gets(mem_arr[no]);	
}
...
void select_menu(){
	...
	switch(menu){
      ...
      // interesting, we get a single byte overflow on the stack. this means
      // we can overwrite a single byte of ebp and pivot the stack?
...
void* mmap_s(void* addr, size_t length, int prot, int flags, int fd, off_t offset){
    ...
    // & to zero out lowest 3 bits
    // | to add 8 to most significant bit to not map into heap area
